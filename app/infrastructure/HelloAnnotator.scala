      ...
      docs.foreach { case (path,(rev,doc)) =>
        def parse(what: String): List[Annotation] = {
          what.toUpperCase().indexOf("HELLO") match {
	        ...
	        case 0  => models.collab.Annotate(5,scala.collection.immutable.Map("class" -> "keyword"))::parse(what.drop(5))
	        case n  => Plain(n)::models.collab.Annotate(5,scala.collection.immutable.Map("class" -> "keyword"))::parse(what.drop(n+5))
	      }          
        }        
