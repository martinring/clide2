...
create table "sessions" ("id" BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) NOT NULL PRIMARY KEY,"name" VARCHAR NOT NULL,"project" BIGINT NOT NULL,"active" BOOLEAN NOT NULL);
create table "users" ("name" VARCHAR NOT NULL PRIMARY KEY,"email" VARCHAR NOT NULL,"password" VARCHAR NOT NULL);
alter table "openFiles" add constraint "fk_fileInfos_parent" foreign key("parent") references "openFiles"("id") on update CASCADE on delete CASCADE;
alter table "openFiles" add constraint "fk_fileInfos_project" foreign key("projectId") references "projects"("id") on update CASCADE on delete CASCADE;
...
alter table "revisions" add constraint "fk_revision_file" foreign key("fileId") references "openFiles"("id") on update CASCADE on delete CASCADE;
alter table "sessions" add constraint "ky_session_user" foreign key("name") references "users"("name") on update CASCADE on delete CASCADE;
...
alter table "sessions" drop constraint "ky_session_user";
