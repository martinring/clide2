...
object HTML5 {
  ...
  object on {
    @inline def click(elem: HTMLElement, handler: => Unit) =
      elem.addEventListener("click", (e: Event) => handler)
    @inline def submit(elem: HTMLElement, handler: => Unit) =
      elem.addEventListener("submit", (e: Event) => { e.preventDefault(); handler })
    @inline def input(elem: HTMLElement, handler: => Unit) =
      elem.addEventListener("input", (e: Event) => handler)
    @inline def toggle(elem: HTMLInputElement, handler: Boolean => Unit) =
      elem.addEventListener("change", (e: Event) => { handler(elem.checked) })
  }
  
  object bind {
    @inline def checked(elem: HTMLInputElement, v: Bindable[Boolean])(implicit ec: ExecutionContext) = {
      elem.checked = v.get
  ...
  implicit class RichEvent(val event: clide.reactive.Event[_]) extends AnyVal {
    def asText(implicit ec: ExecutionContext) = 
      event.map(x => document.createTextNode(x.toString))
  }
  
  implicit class RichHTMLElement(val elem: HTMLElement) extends AnyVal {
    @inline def text_=(value: String)  = elem.textContent = value
    @inline def text: String           = elem.textContent
    @inline def class_=(value: String) = elem.className = value
    @inline def `class`: String        = elem.className    
    @inline def role_=(value: String)  = elem.setAttribute("role", value)
    @inline def role: String           = elem.getAttribute("role")
    
    @inline def +=(other: Node): Unit = elem.appendChild(other)
    @inline def +=(text: String): Unit = elem.appendChild(document.createTextNode(text))
    @inline def +=(event: clide.reactive.Event[Node])(implicit ec: ExecutionContext): Unit = {
    ...
    @inline def +=(xs: TraversableOnce[HTMLElement]): Unit = {
      for (child <- xs) {
        elem.appendChild(child)
      }
    }    
    @inline def +=[T <: HTMLElement](buf: ObservableBufferView[T])(implicit ec: ExecutionContext): Unit = {
