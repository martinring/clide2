...
subsection{* The Function @{term delete} *}

text{*
  The function @{term delete} is an optimisation. It starts from the observation that 
  ...
  argument.The @{term delete} fuction adds a delete action, but commutes it as far back 
  into the argument as possible.
  *}  

fun delete :: "'char operation \<Rightarrow> 'char operation"
where
  "delete (Insert c# next) = Insert c# delete next"
| "delete as = Delete# as"

fun insert :: "'char \<Rightarrow> 'char operation \<Rightarrow> 'char operation"
where
  "insert c as = Insert c#as"

text{* 
  After two auxiliary lemmas  we will show that @{term delete} really adds a delete action,
  in the sense that if we apply it then it removes the first element of the document.
  *}
 
lemma deleteValid11[rule_format]: "((a,d),d') \<in> application \<Longrightarrow> \<forall>c. ((delete a,c#d),d') \<in> application"
  by (rule application.induct, auto)

lemma deleteValid1: "((Delete#as,d),d') \<in> application \<Longrightarrow> ((delete as,d),d') \<in> application"
  by (smt action.distinct(3) action.distinct(5) deleteValid11 application.cases list.distinct(1) list.inject)

lemma deleteValid: "applyOp (delete a) (c#d) = applyOp a d"
  by (rule delete.induct, auto)

text{* 
  As can be expected, @{term delete} does not change the @{term outputLength}. 
  This lemma is actually not needed in the following.
  *}
lemma deleteOutputLength: "outputLength (delete as) = outputLength as"
  by (rule delete.induct, auto)
...
lemma compositionInv [rule_format]: "((a,b),ab) \<in> composition \<Longrightarrow> \<forall>d d' d''. ((a,d),d') \<in> application \<longrightarrow> ((b,d'),d'') \<in> application \<longrightarrow> ((ab,d),d'') \<in> application"
  ...
  apply (auto simp add: emptyInput deleteValid11 remDelete remInsert remRetain)
...
| "normalize (Delete#a) = delete (normalize a)"
...
lemma deleteNormalized[rule_format]: "normalized a \<longrightarrow> normalized (delete a)"
  by (induct a rule: delete.induct, simp_all)

lemma normalizes: "normalized (normalize a)"
  by (induct a rule: normalize.induct, simp_all add: deleteNormalized)
...
lemma composeNormalized[rule_format]: "\<forall>ab. compose a b = Some ab \<longrightarrow> normalized ab"
  by (rule compose.induct, auto intro:  deleteNormalized)
...
(*| type_constructor "list" \<rightharpoonup> (Scala) "Seq[(_)]"
| constant "Nil" \<rightharpoonup> (Scala) "!Seq()"
| constant "Cons" \<rightharpoonup> (Scala) "Seq(_,_)"*)

export_code compose transform in Scala
  module_name SimpleOperation file "../../../scala/clide/collaboration/SimpleOperation.scala"
