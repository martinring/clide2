...
datatype 'char action = Retain | Insert 'char | Delete 'char
...
fun inputLength :: "'char operation \<Rightarrow> nat" where
  "inputLength [] = 0"
| "inputLength (Retain#as) = 1 + inputLength as"
| "inputLength (Delete _#as) = 1 + inputLength as"
...
| "outputLength (Delete _#as)   = outputLength as"

text {* if an operation @{term a} has output lenght 0, a must be empty or is consists only of delete 
        actions *}

lemma emptyOutputDelete [rule_format]: "outputLength a = 0 \<longrightarrow> a = [] \<or> (\<exists>as c. a = Delete c#as)"
...
| "applyOp (Delete c#next) (c'#tail)   = (if (c = c') then applyOp next tail else None)"
| "applyOp (_)             (_)         = None"
...
| delete[intro!]: "((a,d),d') \<in> application \<Longrightarrow> (((Delete c)#a,c#d),d')     \<in> application"
...
lemma applyOpSet1 [rule_format]: "\<forall>d'. applyOp a d = Some d' \<longrightarrow> ((a,d),d') \<in> application"
  by (rule applyOp.induct, auto)

lemma applyOpSet2: "((a,d),d') \<in> application \<Longrightarrow> applyOp a d = Some d'"
  by (erule application.induct, auto)

lemma applyOpSet: "applyOp a d = Some d' \<longleftrightarrow> ((a,d),d') \<in> application"
  by (auto, simp add: applyOpSet1, simp add: applyOpSet2)
...
lemma remRetain: "(((Retain#as,d),d') \<in> application) = (\<exists>c cs cs'. d = c#cs \<and> d'=c#cs' \<and> ((as,cs),cs') \<in> application)"
  apply (auto)
...
lemma remDelete: "(((Delete c # list, d),d') \<in> application) = 
                  (\<exists>cs. d = c#cs \<and> ((list,cs),d') \<in> application)"
  apply (auto)  
  by (smt action.distinct(3) action.inject(2) application.simps inputLength.simps(3) inputLength.simps(4) list.distinct(1) list.inject)
...
lemma applicationDomain: "(\<exists>d'. applyOp a d = Some d') \<longrightarrow> (inputLength a = length d)"
...
fun addDeleteOp :: "'char \<Rightarrow> 'char operation \<Rightarrow> 'char operation"
where
  "addDeleteOp c' (Insert c#next) = (Insert c)#(addDeleteOp c' next)"
| "addDeleteOp c' as = Delete c'#as"

lemma addDeleteOpValid11: "((a,d),d') \<in> application \<Longrightarrow> \<forall>c. ((addDeleteOp c a,c#d),d') \<in> application"
  by (rule application.induct, auto)

lemma addDeleteOpValid1: "((Delete c#as,d),d') \<in> application \<Longrightarrow> ((addDeleteOp c as,d),d') \<in> application"  
  by (smt action.distinct(3) action.distinct(5) action.inject(2) addDeleteOpValid11 application.simps list.distinct(1) list.inject)

lemma addDeleteOpValid: "applyOp a d = Some d' \<Longrightarrow> \<forall>c. applyOp (addDeleteOp c a) (c#d) = Some d'"
  by (simp add: applyOpSet addDeleteOpValid11)

lemma addDeleteOutputLenght[simp]: "outputLength (addDeleteOp c as) = outputLength as"
...
| "normalize (Delete c#a) = addDeleteOp c (normalize a)"
...
lemma normalizeAddDeleteOp[rule_format]: "normalize a = a \<longrightarrow> addDeleteOp c a = normalize (addDeleteOp c a)"
  apply (induct_tac a)
  apply (simp)
  apply (case_tac a, simp_all)
  done

lemma normalizeAddDeleteOp2[rule_format]: "normalize a = a \<longrightarrow> addDeleteOp c a = normalize (Delete c#a)"
  apply (induct_tac a)
  apply (simp)
  apply (case_tac a, simp_all)
  done
...
where
  "compose [] []                         = Some []"
| "compose (Delete c#as)  (bs)           = Option.map (addDeleteOp c) (compose as bs)"
...
| "compose (Retain#as)    (Delete c#bs)  = Option.map (addDeleteOp c) (compose as bs)"
| "compose (Insert(c)#as) (Retain#bs)    = Option.map (Cons (Insert c)) (compose as bs)"
| "compose (Insert(c)#as) (Delete c'#bs) = (if c = c' then compose as bs else None)"
| "compose _              _              = None"
...
| [intro!]: "((a,b),ab) \<in> composition \<Longrightarrow> ((Delete c#a,b),addDeleteOp c ab) \<in> composition"
...
| [intro!]: "((a,b),ab) \<in> composition \<Longrightarrow> ((Retain#a,Delete c#b),addDeleteOp c ab) \<in> composition"
| [intro!]: "((a,b),ab) \<in> composition \<Longrightarrow> ((Insert c#a,Retain#b),Insert c#ab) \<in> composition"
| [intro!]: "((a,b),ab) \<in> composition \<Longrightarrow> ((Insert c#a,Delete c#b),ab) \<in> composition"
...
| "transform (Delete c#as)  (Delete c'#bs) = (if c = c' then transform as bs else None)"
| "transform (Retain#as)    (Delete c#bs)  = Option.map (\<lambda>(at,bt). (at,Delete c#bt)) (transform as bs)"
| "transform (Delete c#as)  (Retain#bs)    = Option.map (\<lambda>(at,bt). (Delete c#at,bt)) (transform as bs)"
| "transform _              _              = None"

text {* the transformation of two operations yields a result if they can be applied to the same documents *}

lemma transformDomain1: "applyOp a d \<noteq> None \<and> applyOp b d \<noteq> None \<longrightarrow> (\<exists>ab. transform a b = Some ab)"
  apply (induct_tac a d rule: applyOp.induct, auto) oops

lemma transformDomain2: "(\<exists>ab. transform a b = Some ab) \<longrightarrow> inputLength a = inputLength b"
...
| [intro!]: "((Delete c#a,b),(a',b')) \<in> transformation \<Longrightarrow> ((Delete c#a,Insert c'#b),(Retain#a',Insert c'#b')) \<in> transformation"
| [intro!]: "((a,b),(a',b')) \<in> transformation \<Longrightarrow> ((Retain#a,Retain#b),(Retain#a',Retain#b')) \<in> transformation"
| [intro!]: "((a,b),(a',b')) \<in> transformation \<Longrightarrow> ((Delete c#a,Delete c#b),(a',b')) \<in> transformation"
| [intro!]: "((a,b),(a',b')) \<in> transformation \<Longrightarrow> ((Retain#a,Delete c#b),(a',Delete c#b')) \<in> transformation"
| [intro!]: "((a,b),(a',b')) \<in> transformation \<Longrightarrow> ((Delete c#a,Retain#b),(Delete c#a',b')) \<in> transformation"
...
lemma tp2: "transform a b = Some(a',b') \<Longrightarrow> compose a b' = Some ab' \<Longrightarrow> compose b a' = Some ba' \<Longrightarrow> transform c ab' = transform c ba'"    
  by (simp add: tp1)

fun inverse :: "'char operation \<Rightarrow> 'char operation" where
  "inverse [] = []"
| "inverse (Retain#xs) = Retain#(inverse xs)"
| "inverse (Insert c#xs) = (Delete c)#(inverse xs)"
| "inverse (Delete c#xs) = (Insert c)#(inverse xs)"

lemma insertCompose[rule_format]: "compose a b = Some y \<longrightarrow> (\<exists>y. compose a (Insert c#b) = Some y)"
  apply (induct_tac a b rule: compose.induct, auto)  
  by (metis composeSet11 option.exhaust)

lemma composeInverse: "\<exists>e. (compose a (inverse a)) = Some e"
  by (induct_tac a rule: inverse.induct, auto simp add: insertCompose)
